...
module.exports = function() {
    ...
    this.wsServer = null;
    this.wsPath = "/one2one";
    this.logWsMessages = true;

    this.UserSession = UserSession;

    this.clearCandidatesQueue = clearCandidatesQueue;
    this.getKurentoClient = getKurentoClient;
    ...
    */
    ...
    var userRegistry = this.userRegistry = new UserRegistry();
    var pipelines = this.pipelines = {};
    var candidatesQueue = this.candidatesQueue = {};
    var idCounter = 0;
        ...
        getKurentoClient(function(error, kurentoClient) {
            ...
            kurentoClient.create('MediaPipeline', function(error, pipeline) {
                ...
                pipeline.create('WebRtcEndpoint', function(error, callerWebRtcEndpoint) {
                    ...
                    if(self.streamBitrateLimits) {
                        callerWebRtcEndpoint.setMaxVideoRecvBandwidth(self.streamBitrateLimits.video);
                        callerWebRtcEndpoint.setMaxAudioRecvBandwidth(self.streamBitrateLimits.audio);
                    }
                    ...
                    pipeline.create('WebRtcEndpoint', function(error, calleeWebRtcEndpoint) {
                        ...
                        if(self.streamBitrateLimits) {
                            calleeWebRtcEndpoint.setMaxVideoRecvBandwidth(self.streamBitrateLimits.video);
                            calleeWebRtcEndpoint.setMaxAudioRecvBandwidth(self.streamBitrateLimits.audio);
                        }
    ...
    */
    ...
    function startServer(callback) {
      ...
      var port = asUrl.port;
      var options = self.serverOptions || {
          key:  fs.readFileSync('keys/server.key'),
          cert: fs.readFileSync('keys/server.crt')
      }
      var server = https.createServer(options, app).listen(port, callback || function() {
          console.log('Kurento Tutorial started');
          console.log('Open ' + url.format(asUrl) + ' with a WebRTC capable browser');
      });

      var wss = this.wsServer = new ws.Server({
          server : server,
          path : self.wsPath
      });
